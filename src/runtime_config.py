...
SPECS = (
    ...
    ConfigSpec("MONITOR_INTERVAL_MINUTES", 45, int, "local_bot.py, src/post.py"),
    ConfigSpec("ACTIVE_HOURS", "5-23", str, "local_bot.py, src/post.py"),
    ConfigSpec("MAX_POSTS_PER_RUN", 1, int, "src/post.py", "Phase Aでは1を維持"),
    ConfigSpec("ORIGINAL_DAILY_POST_MIN", 20, int, "daily target monitoring", "品質・安全ゲートを維持する目標値"),
    ConfigSpec("ORIGINAL_DAILY_POST_MAX", 20, int, "src/post.py", "通常投稿の上限"),
    ConfigSpec("MAX_DAILY_AUTOMATED_POSTS", 20, int, "src/post.py", "速報を含む総投稿上限"),
    ConfigSpec("MIN_POST_INTERVAL_MINUTES", 45, int, "src/post.py", "20件目標に対応する最短間隔"),
